...
12. Оформление графиков с **seaborn** и **matplotlib**
    ...
    <img alt="ROC curve" src="/img/plots_3.png" height="500">

13. Логирование с помощью чат-бота в Телеграм.
<img alt="логирование" src="/img/log.jpg" height="500">
...
изменть настройки подключения при инициализации `RDBStorage`. Также можно подключить к 
...
- создать окружение из файла **requirements.yml** с помощью команды `conda env create -f requirements.yml` 
(предполагается, что коммандная строка запущена из каталога с **requirements.yml**), созданное окружение
будет называться `titanic`;
...
- в командной строек появится сылка на сервер *jupyter*, необходимо прейти по ней, 
чтобы попасть на сервер *jupyter*.
